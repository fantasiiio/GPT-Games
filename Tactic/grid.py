...
class Tile:
    ...
    def get_screen_rect(self):
        return pygame.Rect(
            self.x * self.grid.tile_size + self.grid.get_camera_screen_position()[0],
            self.y * self.grid.tile_size + self.grid.get_camera_screen_position()[1],
            self.grid.tile_size,
            self.grid.tile_size,
        )
...
class Grid:
    ...
    def get_camera_screen_position(self):
        return (
            self.camera_world_position[0] + self.screen.get_rect().width / 2,
            self.camera_world_position[1] + self.screen.get_rect().height / 2,
        )
    ...
    def set_camera_screen_position(self, x, y):
        self.camera_world_position = (
            ...
            y - self.screen.get_rect().height / 2,
        )
        self.camera_world_position = self.camera_world_position

    def get_tiles_with_property(self, property_name):
        tiles = []
    ...
    def load_tmx_map(self, filename):
        ...
        self.tiles = [
            [Tile(x, y, self) for y in range(tmx_data.height)]
    ...
    def get_next_point_within_max_range(self, start_point, next_straight_point, max_range):
        ...
        if dx == 0:
            ...
            next_point = (
                start_point[0],
                start_point[1] + max_range if dy > 0 else start_point[1] - max_range,
            )
        elif dy == 0:
            # Horizontal movement
            next_point = (
                start_point[0] + max_range if dx > 0 else start_point[0] - max_range,
                start_point[1],
            )
        else:
            ...
            next_point = (
                (
                    start_point[0] + max_range
                    if dx > 0
                    else start_point[0] - max_range,
                    start_point[1],
                )
                if abs(dx) > abs(dy)
                else (
                    start_point[0],
                    start_point[1] + max_range
                    if dy > 0
                    else start_point[1] - max_range,
                )
            )
    ...
    def draw_text_with_border(
        self, screen, text, font, x, y, text_color, border_color, border_thickness=1
    ):
    ...
    def draw_path(self, path, color):
        ...
        for index, tile in enumerate(path):
            ...
            highlight_color = (
                ...
                color[2],
                110,
            )  # Example: more opaque color for tiles with units
            ...
            self.screen.blit(
                highlight_surface,
                (
                    tile[0] * self.tile_size + self.get_camera_screen_position()[0],
                    tile[1] * self.tile_size + self.get_camera_screen_position()[1],
                ),
            )
            # pygame.draw.rect(self.screen, (255, 0, 0), (tile[0]*self.tile_width, tile[1]*self.tile_width, self.tile_width, self.tile_width), 2)
    ...
    def update(self, inputs):
        ...
        if inputs.mouse.clicked[0]:
            if self.mouse_over_tile != selected_tile:
                if (
                    selected_tile
                    and selected_tile.unit
                    and selected_tile.unit.current_action is not None
                ):
                    return
                if not (
                    self.mouse_over_tile.unit
                    and self.mouse_over_tile.unit.current_action != None
                ):
        ...
        if inputs.mouse.clicked[1]:
            ...
            if self.clicked_position:
                ...
                self.set_camera_world_position(
                    self.camera_start[0] + offset[0], self.camera_start[1] + offset[1]
                )

    def highlight_tiles(self, unit_position, range_count, color, action):
        self.highlighted_tiles = []

        tiles = self.tiles_in_range_manhattan(unit_position[0], unit_position[1], range_count)
        for tile_pos in tiles:
            if self.position_is_in_grid(
                tile_pos[0], tile_pos[1]
            ):  # corrected typo: posision -> position
                ...
                if self.is_passable(tile.x, tile.y, action):
                    ...
                    self.screen.blit(
                        ...
                        (
                            ...
                            + self.get_camera_screen_position()[0],
                            tile.y * self.tile_size
                            + self.get_camera_screen_position()[1],
                        ),
                    )
    ...
    def is_passable(self, x, y, action):
        ...
        if action == "choosing_move_target" and tile.unit is not None:
            if (
                tile.unit
                and self.selected_tile.unit
                and tile.unit.player == self.selected_tile.unit.player
            ):
                if (
                    (tile.unit.type == "Tank" and tile.unit.seat_left > 0)
                    or (tile.unit.type == "Boat" and tile.unit.seat_left > 0)
                    or (tile.unit.type == "Helicopter" and tile.unit.seat_left > 0)
                ):
        ...
        if (
            action == "choosing_move_target"
            and self.selected_tile.unit
            and self.selected_tile.unit.type == "Boat"
        ):
            if not tile.properties or not tile.properties["type"] == "water":
                return False

        if (
            action == "choosing_fire_target"
            and self.selected_tile.unit
            and tile.unit is not None
            and tile.unit.player == self.selected_tile.unit.player
        ):
            return False
        ...
        if action == "choosing_move_target" and (
            not self.selected_tile.unit or self.selected_tile.unit.type != "Boat"
        ):
            if tile.no_walk:
                return False
    ...
    def move_camera_to(self, target):
        ...
        self.camera_move_start_time = pygame.time.get_ticks()
        self.camera_direction_vector = (
            target[0] + self.camera_start[0],
            target[1] + self.camera_start[1],
        )
    ...
    def move_camera_to_tile(self, tile):
        self.move_camera_to(
            (
                tile.x * self.tile_size + self.get_camera_screen_position()[0],
                tile.y * self.tile_size + self.get_camera_screen_position()[1],
            )
        )
    ...
    def draw_grid(self, inputs, darken_other_side=True):
        ...
        selected_tile_coords = (
            (selected_tile.x, selected_tile.y) if selected_tile else None
        )
        ...
        for x in range(min_x, max_x):
            for y in range(min_y, max_y):
                ...
                pygame.draw.rect(
                    self.screen,
                    GRAY,
                    (tile_pos_x, tile_pos_y, self.tile_size, self.tile_size),
                    1,
                )

                if self.mouse_over_tile and (
                    self.mouse_over_tile.x,
                    self.mouse_over_tile.y,
                ) == (x, y):
                    pygame.draw.rect(
                        self.screen,
                        (255, 255, 255),
        ...
        if (
            selected_tile
            and selected_tile.unit
            and selected_tile.unit.current_action == "choosing_move_target"
        ):
            x, y = inputs.mouse.pos
            clicked_tile = self.get_tile_from_coords(x, y)
            path = self.manhattan_path(
                (selected_tile.x, selected_tile.y), (clicked_tile.x, clicked_tile.y)
            )
        ...
        if darken_other_side:
            ...
            if self.current_player == 1:
                start_x = (
                    middle_x * self.tile_size + self.get_camera_screen_position()[0]
                )
        ...
        for x in range(self.tiles_x):
            for y in range(self.tiles_y):
                ...
                if tile.unit:
                    continue
                if tile.properties:
                    type_name = tile.properties.get("type")
                    if type_name == "water":
    ...
    def find_path(self, start, end):
        ...
        queue = deque(
            [(start, [start])]
        )  # Queue for BFS. Each element is a tuple containing current cell coordinates and the path taken to reach there.
    ...
    def get_farthest_walkable_point(self, path, max_range):
        ...
        for _, point in enumerate(path[1:]):
