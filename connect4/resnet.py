    ...
    def generate_data(self, num_games):
        ...
        for _ in range(num_games):
            ...
            while not node.is_terminal():
                ...
                new_node.drop_piece(new_node.last_move, new_node.player)
                node = new_node
            winner = node.player
            for state, action in game_history:
                data.append((state, action, winner))
        return data
    ...
    def train_resnet(self, data, epochs=5):
        for epoch in range(epochs):
            for state, action, winner in data:
                ...
                loss_policy = self.criterion_policy(policy, action)
                loss_value = self.criterion_value(value, torch.tensor([winner], dtype=torch.float32))
                loss = loss_policy + loss_value

                loss.backward()
                self.optimizer.step()

    def self_play_train(self, iterations=10, num_games=100, epochs=5):
